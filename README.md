...
> 20190423 v1.05 更新：添加了js中修改css属性的适配，修复touchstart和touchend事件能多点触发的bug，添加 touchcancel 事件

> 20190223 v1.04 更新：给用于改变 carousel 的 `setId`方法做了性能优化(此方法较消耗性能)，防止使用者滥用此方法造成性能上的损失，具体的做法是：1. 当调用时传入的id就是当前滑轮的id时，避免不必要的操作 2. 包装节流函数,使得频繁调用此方法时，以一个固定的间隔触发此方法，间隔设置在常量定义中，为200ms
